...
//功能：从当前位置startpos（数据采样当前位置，数据最新）往回计算最新的 AVGDataNum个数据的平均值

//*p_totalcnt 循环区有效的准备好的参与平均值的数据的总数
//AVGDataNum: 求平均值的  样板数据个数
//buf 缓冲区指针
//BUFSIZE ：整个缓冲区的长度
//startpos : 数据开始点 （往回）
uint16_t Cyclic_Aver(uint16_t* buf, uint8_t BUFSIZE, uint8_t startpos,
		uint8_t AVGDataNum, volatile uint8_t* p_totalCnt);
/* 计算两条直线的K与b值 */
void
Calc_Virtual_KandB(void);

//计算缓冲区平均值
uint32_t Util_CalcAvg(uint16_t *buf, uint8_t length)
{
	uint8_t i;
	uint32_t Vavg = 0;
	for (i = 0; i < length; i++)
...
//orgv :实测的原始点压
//返回 温度校准后的电压
uint16_t CalTempCalibrate(uint16_t orgv)
{
	float_t k, deltVcal;
	uint16_t tmpcalib;
...
#endif
	deltVcal = -k * ((float) Vtemp_Aver1S - gTemp0) + orgv;
	if (deltVcal < 0)
		deltVcal = 0;
...
void UART0_IRQHandler(void)
{
	...
	}
	{
		...
		if (CommandReceived)
			memcpy((char *) &ReadBuffer, (char *) &SerialBuffer,
					SERIAL_BUFF_SIZE);
...
void Calc_Virtual_KandB(void)
{
	...
	{
		VB1 = Calibrate_B_AD
				- (Calibrate_B_Concentration * (Sensitivity * Kn1));
	}
	else
	{
		VB1 = CleanVoltage;
	...
	{
		VB2 = Calibrate_B_AD
				- (Calibrate_B_Concentration * (Sensitivity * Kn2));
	}
	else
	{
		VB2 = CleanVoltage;
		...
		uint8_t AVGDataNum, volatile uint8_t* p_totalCnt)
{
	...
	for (i = 0; i < k; i++)
	{
		...
		{
			startpos = BUFSIZE - 1; //回到最新的最末端缓冲数据处
		}
		result += buf[startpos];
	}
	result /= k;
...
void Getdisplevel(void *p1)
{
	...
	sprintf(buf, S_DISPOINT_ALL, Ind_DustP1, Ind_DustP2, Ind_DustP3,
			Ind_DustP4);
	Uartcmd(buf);
}
...
_Bool GetMachineWorking(void)
{
	...
	{
		up = TRUE;
		//	Readed_Machine_State = 0x05;  //下次如果上位机不发信息，那状态就超时处理
	}
	//很久没有收到状态指令信息 working不更新，就会超时，且工作模式为串口
	if ((workingTm == 0) && (DustPortOrUartMode == 1))
		up = 0;
	return up;
}
#ifdef REF25_VDD
...
#endif
//计算1秒的灰尘采样平均值
void CalDustVolTageAVG()
{
	char strVal[LCD_WIDTH] = "Sec_Value:      ";
	uint16_t sec_Aver;
	// 计算秒均值  浓度
	sec_Aver = Util_CalcAvg((uint16_t*) Sec_Data, DATA_SIZE);
#ifdef  REF25_VDD
...
//每秒调用一次
//向下更新
void CalTurnDown(uint16_t DustV)
{
	uint32_t temp;
...
//以下介于3% -10% 才更新新，
	...
	if (++TurnDownCleanVolConfirmTimes >= CLEAN_DOWN_CONFIRM_TIMES)
	{
		...
		CleanVoltage = DustV; //??????????????????????????????????????还是去  CyclicAver  CyclicAveraftercalibrate
		if (Calibrate_A_Concentration == 0)
		{
