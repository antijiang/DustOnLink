...
volatile uint16_t Sec_Data[DATA_SIZE], Min_Data[60], Update_Data[UPDATE_CYCLIC_COUNT], Dust_N_Data[CYCLIC_TIME];
#ifdef	REF25_VDD
...
volatile uint8_t TurnDownCleanVolConfirmTimes = 0;
...
uint32_t Calc_Dust_Aver(uint16_t* dustData, uint8_t length, uint16_t secAver);
...
//  uint16_t calaft;
  ...
  if (gTempn==gTemp0)return;

  k=((float) gTempnV-gTemp0V)/((float)gTempn-gTemp0);

  if (gTempnV==0xffff||gTemp0V==0xffff||gTempn==0xffff||gTemp0==0xffff)return;
  deltVcal=-k*(Vtemp_Aver1S-gTemp0)+VB1;
  if (deltVcal<0)deltVcal=0;
  ...
  if (deltVcal<0)deltVcal=0;
  CyclicAveraftercalibrate= (uint16_t)deltVcal;
...
#else
  SYS->P1_MFP |= SYS_MFP_P10_AIN0 | SYS_MFP_P13_AIN3|SYS_MFP_P14_AIN4 |SYS_MFP_P17_AIN7;
#endif
  ...
  if (P0->ISRC&(1<<7))
    {
      ...
      if (PulseCNT!=0xffffffff)PulseCNT++;
    }
...
void UART0_IRQHandler(void)
{
    ...
    }
  {
    ...
    if (CommandReceived)	memcpy((char *)&ReadBuffer,(char *)&SerialBuffer,SERIAL_BUFF_SIZE);
...
{
  UART0->BAUD = UART_BAUD_MODE2 | UART_BAUD_DIV_MODE2(__IRC22M, UART_BAUD_RATE);
...
#endif
#ifdef	ADDR_USE_SWITCH
void PWMLEDO_init(void) //使用PWMB中断
{
...
void AdaptTMRDelay(void)
{
  // 停止定时器, 丢弃当前值.
  _TIMER_RESET(TIMER0);
  TIMER0->TCMPR = __IRC22M / 1000.0 * (10 * (1 - PWM_DEFAULT_DUTY_RATIO / (pow(2, Gear))) +
...
void StartTMR(void)
{
  ...
  TIMER0->TCSR = TIMER_TCSR_CEN_Msk | TIMER_TCSR_IE_Msk | TIMER_TCSR_MODE_ONESHOT | TIMER_TCSR_TDR_EN_Msk |
                 TIMER_TCSR_PRESCALE(1);
}

void SendCalInfoTemp()
{
...
void ACK(char err)
{
  ...
  if (err==0)	sprintf(buf,"%s","OK\r");
  else  sprintf(buf,"%s\r","ERR");
  Uartcmd(buf);

}
const char S_AUTOMODE[]="AMODE:%d\r";
void GetAutoMode(void *p1)
{
  ...
  if (p1!=NULL)
    {
      ...
      a=atoi((char *)p1);
      if (a==1)	AutoOutPutCurrentSmoke = TRUE;
      else if (a==0)AutoOutPutCurrentSmoke = FALSE;
      else err=1;
    }
  else err=1;
  ACK(err);
  ...
  if (p1!=NULL)
    {
      ...
      a=atoi((char *)p1);
      if (a==1)DustPortOrUartMode=1;
      else if (a==0) DustPortOrUartMode=0;
      else err=1;
    }
  else err=1;
  ACK(err);
  ...
  if (p1!=NULL)
    {
      ...
      a=atoi((char *)p1);
      if (a==1) Readed_Machine_State =1;
      else if (a==0)  Readed_Machine_State = 0;
      else err=1;
    }
  else err=1;
  ACK(err);
  ...
  if (p1!=NULL)
    {
      ...
      else CurrentUARTMod=UARTMOD_DATA_LINK;
    }
  else err=1;
  ACK(err);
...
void ProcessInterfaceCommand(void)
{
...
void TryUpdateClean(uint16_t value)
{
  if (TurnUpCleanVolConfirmTimes >= CLEAN_UP_CONFIRM_TIMES)
    {
      if ((value < First_Value) && (((First_Value - value) / (float)First_Value) > CLEAN_UP_FINAL_RANGE))
        {
          uint16_t subValue = ((Last_Value - CleanVoltage) * CLEAN_UP_CALI_VALUE);
...
#ifdef REF25_VDD
      // 计算秒均值VDD
      for (i = 0,sec_Count=0; i < DATA_SIZE; i++)
        {
          sec_Count += Vdd_Data[i];
        }
...
#ifdef 		REF25_VDD
              //电源电压过低，不做更新无尘电压  //jxd
              if (VddRel_MV<MIN_UPDATE_VDD) //4.8v
                {
                  TurnDownCleanVolConfirmTimes=0;
                }
              else
      ...
      if (Update_ReadCount == UPDATE_CYCLIC_COUNT - 1)
        {
          ...
          if (DustPortOrUartMode==0) //jxd利用端口判定状态
            {
              if (DUSTIN_STATUS==1)Readed_Machine_State=1;
              else Readed_Machine_State=0;
      ...
      if (Min_ReadCount >= 60)
        {
          ...
          for (i = 0; i < 60; i++)
            {
              min_Count += Min_Data[i];
            }
          min_Aver = min_Count / 60;
          Min_ReadCount = 0;
      ...
      if (Update_ReadCount >= UPDATE_CYCLIC_COUNT)
        {
          ...
          uint8_t i;
          uint32_t min_Count = 0;
          for (i = 0; i < Update_ReadCount; i++)
            {
              min_Count += Update_Data[i];
            }
          min_Aver = min_Count / UPDATE_CYCLIC_COUNT;
          Update_ReadCount = 0;

          if (CurrentUARTMod == UARTMOD_CAL)
            {
              Current_Machine_State=0;
            }
          else
            {
              // 提高无尘电压
...
#endif
              // 如果灰尘值大于一定值
              if (min_Aver > CleanVoltage + CLEAN_UP_UPDATE_THRESHOLD * Sensitivity)
                {
                  // 与最后一次获取值相差的百分比
                  float percent = (min_Aver > Last_Value)?
                                  ((min_Aver - Last_Value) / (float)Last_Value):
                                  ((Last_Value - min_Aver) / (float)Last_Value);
                  if (First_Start)
                    {
                      First_Value = min_Aver;
                      First_Start = FALSE;
                    }
                  if (First_Value < min_Aver)
                    First_Value = min_Aver;
                  // 设置最后获取到的值
